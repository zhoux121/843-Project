...
class SIFTDataset(Dataset):
    ...
    def load_item(self, idx):
        Ig = cv.imread(self.file + self.filelist[idx])
        Ig = cv.resize(Ig, (256, 256))
    ...
    def load_sift(self, img):
        ...
        meta, des = affine_detect(detector, np.uint8(img), pool=pool)
        meta1, des1 = sift.detectAndCompute(np.uint8(img), None)
        newk_list = []
        newd_list = []
        pts = cv.KeyPoint_convert(meta)
        ptsx = pts[:,0]
        ptsy = pts[:,1]
        ...
        for i in range(len(ptsx)):
            for j in range(len(pts1x)):
                if (ptsx[i]==pts1x[j]) and (ptsy[i]==pts1y[j]):
                    ...
                    newd_list.append(des[i])
        meta = cv.KeyPoint_convert(newk_list)
    ...
    def load_edge(self, img, masking=False):
        ...
        return canny(cv.cvtColor(img, cv.COLOR_BGR2GRAY) / 255.0, sigma=2, mask=mask).astype(float)
...
class LBPModel(BaseModel):
    ...
    def backward(self, gen_loss=None, dis_loss=None, retain_graph=True):
...
class SIFTReconstruction():
    def __init__(self, choice='SLI'):
        ...
        self.n_epochs = 3
        self.train_loader = DataLoader(dataset=train_dataset, batch_size=1, shuffle=True, num_workers=0)
        ...
        for epoch in range(self.n_epochs):
            ...
            for cnt, items in enumerate(self.train_loader):
                ...
                if not self.TRAIN_BOTH:
                    ...
                    history.append(gen_img_loss.item())
    ...
    def test(self, pretrained=False):
        ...
        for cnt, items in enumerate(self.test_loader):
            ...
            if cnt < 100:
                ...
                # cv.imwrite('res/' + self.choice + '_results/Lo_%06d.jpg' % (cnt+1), Lo[0])
                Io = self.postprocess(Io)
                cv.imwrite('res_ASIFT/' + self.choice + '_results/Io_%06d.jpg' % (cnt+1), Io[0])
        if pretrained:
...
if __name__ == '__main__':
    model = SIFTReconstruction('SLI')
    start_time = time.clock()
    #model.test(True)
    history = model.train()
    epochs = range(0,60)
    plt.plot(epochs, np.array(history), 'g', label='Training loss')
    plt.title('Training loss for ASIFT')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.show()
    print((time.clock() - start_time)/10, "seconds")
    print('End.')
